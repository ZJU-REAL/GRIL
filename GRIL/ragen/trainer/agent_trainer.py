...
class RayAgentTrainer(VerlRayPPOTrainer):
    ...
    def _validate(self):
        ...
        for step in range(self.config.trainer.validation_steps):
            ...
            test_batch = self.agent_proxy.rollout(test_gen_batch,step_num=self.global_steps, val=True)
            end_time = time.time()
            ...
            data_source_lst.append(test_batch.non_tensor_batch.get("type", ["metamath_qa"] * reward_tensor.shape[0]))
        ...
        for step in range(self.total_training_steps):
            ...
            with _timer("step", timing_raw):
                ...
                with _timer("gen", timing_raw):
                    batch = self.agent_proxy.rollout(batch,step_num=self.global_steps, val=False)
                    # save_dir = os.path.join(os.getcwd(), "train_rollout_logs")
                    # os.makedirs(save_dir, exist_ok=True)

                    # timestamp = time.strftime("%Y%m%d_%H%M%S")
                    # message_list = batch.non_tensor_batch["messages_list"]

                    # jsonl_path = os.path.join(save_dir, f"messages_list_metamath_mix_{timestamp}.jsonl")
